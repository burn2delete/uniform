...
Two modes are supported.  A public/pure batch acquires the canonical SH-07
lease and runs one of the reviewed Clojure batches.  The authority batch
starts a *new* checkpoint directory and invokes the existing SH-07 checkpoint
runner with exactly ``c7-types`` and ``--no-resume``.  In authority mode the
checkpoint child, rather than this process, owns the shared lease.
"""
...
MODES = (MODE_PURE, MODE_AUTHORITY)
...
def _validate_captured_output_contract(
    *,
    ...
    try:
        ...
        try:
            valid = bool(_sh07.output_contract_passed(
                AUTHORITY_MODULE,
                source_path,
                source_size,
                source_hash,
...
def _authority_manifest_valid(
    ...
    *,
    ...
    if manifest.get("shared_context_fingerprint_after") not in (None, shared_fingerprint):
    ...
    if record.get("exit_code") != 0 or record.get("raw_child_exit_code") not in (None, 0):
        reasons.append("c7-types child exit was not zero")
    if record.get("timed_out") is True or record.get("context_stable") is not True:
    ...
    if not isinstance(command, list) or "--fresh" not in command or AUTHORITY_MODULE not in command:
...
def _run_authority(
    *,
    ...
    try:
        ...
        manifest = _safe_load_json_fd(state_fd, "manifest.json")
        valid, evidence = _authority_manifest_valid(
            root, state_dir, manifest, state_fd=state_fd
        )
    finally:
        os.close(state_fd)
    ...
    if valid and result.returncode == 0 and not result.timed_out and not result.survivors:
...
def run_stage3(
    *,
    ...
    if mode == MODE_AUTHORITY and batch != "authority":
        raise Stage3Error("authority mode requires the reviewed authority batch")
    if mode == MODE_PURE and batch == "authority":
        raise Stage3Error("authority batch requires authority mode")
...
def _environment_configuration(cli: Mapping[str, object] | None = None) -> dict[str, object]:
    ...
    batch = cli.get("batch") or os.environ.get(BATCH_ENV, "authority" if mode == MODE_AUTHORITY else DEFAULT_BATCH)
    return {
        "root": root,
