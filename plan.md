...
- Owner: master coordinator; the current patch adds one internal bounded native runtime provider, focused fixtures/tests, one partial proof record, and seed-boundary accounting. It does not edit `bin/gravity` or enable the public route.
...
- Next gate: replace the host-C provider with a Gravity-authored executable implementation, bind it to the authenticated stage2 compiler packet, provide descriptor-relative execution plus OS-level complete-tree containment, and prove accepted `.gravity` and `.qst` execution and stable rejection through tracked `bin/gravity`. The launcher and runtime-provider primitives remain internal prerequisites and cannot satisfy `:contained-public-native-run` together yet.
