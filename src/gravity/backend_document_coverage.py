"""Phase 07 document-specific backend coverage validation."""
...
from gravity.backend_architecture import backend_diagnostic, validate_backend_file


ROOT = Path(__file__).resolve().parents[2]
REQUIRED_DOCUMENTS = {f"B{index}" for index in range(1, 15)}


class BackendDocumentCoverageError(Exception):
    def __init__(
        self,
        code: str,
        message: str,
    ...
    def to_diagnostic(self) -> dict[str, Any]:
        return {
            ...
            "analyzer_stage": "phase07-backend-document-coverage",
        }


def validate_phase07_document_coverage_file(path: Path) -> dict[str, Any]:
    return validate_phase07_document_coverage_manifest(load_json(path), str(path))


def validate_phase07_document_coverage_manifest(manifest: dict[str, Any], source: str) -> dict[str, Any]:
    if manifest.get("kind") != "phase07-backend-document-coverage-input":
        raise_error(
            "B14-EVIDENCE",
            "document coverage input has the wrong kind",
            {},
            source,
            "phase07-backend-document-coverage-input",
        )
    ...
    if missing or len(records) != 14:
        raise_error("B14-COVERAGE", "Phase 07 coverage must include B1 through B14", {}, source, ",".join(missing))
    accepted = []
    rejected = []
    ...
    return {
        "kind": "phase07-backend-document-coverage-artifact",
        "phase": "07",
...
def validate_accepted(record: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    artifact = validate_backend_file(fixture)
    for field in record.get("accepted", {}).get("required_artifact_fields", []):
        if not artifact.get(field):
            raise_error("B14-ARTIFACT", f"accepted artifact missing field {field}", record, source, field)
    coverage = record.get("coverage_claims", [])
    if not coverage:
        raise_error("B14-COVERAGE", "coverage record lacks claims", record, source, "coverage-claims")
    return {
...
def validate_rejected(record: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    diagnostic = backend_diagnostic(fixture)
    expected = rejected.get("diagnostic")
    if diagnostic is None:
        raise_error("B14-NEGATIVE", "rejected fixture was accepted", record, source, "diagnostic")
...
def require_path(record: dict[str, Any], value: str | None, source: str, fact: str) -> Path:
    if not value:
        raise_error("B14-COVERAGE", "coverage record lacks a path", record, source, fact)
    path = Path(value)
    path = path if path.is_absolute() else ROOT / path
    if not path.exists():
        raise_error("B14-COVERAGE", f"coverage path does not exist: {value}", record, source, fact)
    return path


def raise_error(code: str, message: str, record: dict[str, Any], source: str, missing_fact: str) -> None:
    raise BackendDocumentCoverageError(
        code,
        message,
        record=record,
        source=source,
        missing_fact=missing_fact,
        remediation="Update the Phase 07 document coverage record, fixture, or owning backend validator evidence.",
    )
...
def document_sort_key(document: str | None) -> int:
    if not document or not document.startswith("B"):
        return 0
