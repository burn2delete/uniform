"""Phase 11 document-specific AI coverage validation."""
...
from gravity.ai_agentic import ai_agentic_diagnostic, validate_ai_agentic_file


ROOT = Path(__file__).resolve().parents[2]
REQUIRED_DOCUMENTS = [f"A{index}" for index in range(1, 12)]


class AIDocumentCoverageError(Exception):
    def __init__(
        self,
        code: str,
        message: str,
    ...
    def to_diagnostic(self) -> dict[str, Any]:
        return {
            ...
            "analyzer_stage": "phase11-ai-document-coverage",
        }


def validate_phase11_document_coverage_file(path: Path) -> dict[str, Any]:
    return validate_phase11_document_coverage_manifest(load_json(path), str(path))


def validate_phase11_document_coverage_manifest(manifest: dict[str, Any], source: str) -> dict[str, Any]:
    if manifest.get("kind") != "phase11-ai-document-coverage-input":
        raise_error("AI001", "document coverage input has the wrong kind", {}, source, "phase11-ai-document-coverage-input")
    ...
    if missing or len(records) != 11:
        raise_error("AI001", "Phase 11 coverage must include A1 through A11", {}, source, ",".join(missing))
    accepted = []
    rejected = []
    ...
    return {
        "kind": "phase11-ai-document-coverage-artifact",
        "phase": "11",
...
def validate_accepted(record: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    artifact = validate_ai_agentic_file(fixture)
    document = record.get("document")
    document_artifact = artifact["document_contracts"].get(document)
    if not document_artifact:
        raise_error("AI001", "accepted artifact does not include document", record, source, "document-contract")
    for field in record.get("accepted", {}).get("required_artifact_fields", []):
        if not artifact.get(field):
            raise_error("AI001", f"accepted artifact missing field {field}", record, source, field)
    coverage = record.get("coverage_claims", [])
    if not coverage:
        raise_error("AI001", "coverage record lacks claims", record, source, "coverage-claims")
    return {
        ...
        "agent_id": document_artifact.get("agent_id"),
        "coverage": coverage,
...
def validate_rejected(record: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    diagnostic = ai_agentic_diagnostic(fixture)
    expected = rejected.get("diagnostic")
    if diagnostic is None:
        raise_error("AI001", "rejected fixture was accepted", record, source, "diagnostic")
...
def require_path(record: dict[str, Any], value: str | None, source: str, fact: str) -> Path:
    if not value:
        raise_error("AI001", "coverage record lacks a path", record, source, fact)
    path = Path(value)
    path = path if path.is_absolute() else ROOT / path
    if not path.exists():
        raise_error("AI001", f"coverage path does not exist: {value}", record, source, fact)
    return path


def raise_error(code: str, message: str, record: dict[str, Any], source: str, missing_fact: str) -> None:
    raise AIDocumentCoverageError(
        code,
        message,
        record=record,
        source=source,
        missing_fact=missing_fact,
        remediation="Update the Phase 11 document coverage record, fixture, or AI validator evidence.",
    )
...
def document_sort_key(document: str | None) -> int:
    if not document or not document.startswith("A"):
        return 0
