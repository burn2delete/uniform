"""Phase 08 document-specific runtime coverage validation."""
...
from gravity.runtime_architecture import runtime_diagnostic, validate_runtime_file


ROOT = Path(__file__).resolve().parents[2]
REQUIRED_DOCUMENTS = {f"R{index}" for index in range(1, 13)}


class RuntimeDocumentCoverageError(Exception):
    def __init__(
        self,
        code: str,
        message: str,
    ...
    def to_diagnostic(self) -> dict[str, Any]:
        return {
            ...
            "analyzer_stage": "phase08-runtime-document-coverage",
        }


def validate_phase08_document_coverage_file(path: Path) -> dict[str, Any]:
    return validate_phase08_document_coverage_manifest(load_json(path), str(path))


def validate_phase08_document_coverage_manifest(manifest: dict[str, Any], source: str) -> dict[str, Any]:
    if manifest.get("kind") != "phase08-runtime-document-coverage-input":
        raise_error(
            "R1-MANIFEST",
            "document coverage input has the wrong kind",
            {},
            source,
            "phase08-runtime-document-coverage-input",
        )
    ...
    if missing or len(records) != 12:
        raise_error("R1-MANIFEST", "Phase 08 coverage must include R1 through R12", {}, source, ",".join(missing))
    accepted = []
    rejected = []
    ...
    return {
        "kind": "phase08-runtime-document-coverage-artifact",
        "phase": "08",
...
def validate_accepted(record: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    artifact = validate_runtime_file(fixture)
    for field in record.get("accepted", {}).get("required_artifact_fields", []):
        if not artifact.get(field):
            raise_error("R1-MANIFEST", f"accepted artifact missing field {field}", record, source, field)
    coverage = record.get("coverage_claims", [])
    if not coverage:
        raise_error("R1-MANIFEST", "coverage record lacks claims", record, source, "coverage-claims")
    return {
...
def validate_rejected(record: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    diagnostic = runtime_diagnostic(fixture)
    expected = rejected.get("diagnostic")
    if diagnostic is None:
        raise_error("R1-MANIFEST", "rejected fixture was accepted", record, source, "diagnostic")
...
def require_path(record: dict[str, Any], value: str | None, source: str, fact: str) -> Path:
    if not value:
        raise_error("R1-MANIFEST", "coverage record lacks a path", record, source, fact)
    path = Path(value)
    path = path if path.is_absolute() else ROOT / path
    if not path.exists():
        raise_error("R1-MANIFEST", f"coverage path does not exist: {value}", record, source, fact)
    return path


def raise_error(code: str, message: str, record: dict[str, Any], source: str, missing_fact: str) -> None:
    raise RuntimeDocumentCoverageError(
        code,
        message,
        record=record,
        source=source,
        missing_fact=missing_fact,
        remediation="Update the Phase 08 document coverage record, fixture, or owning runtime validator evidence.",
    )
...
def document_sort_key(document: str | None) -> int:
    if not document or not document.startswith("R"):
        return 0
