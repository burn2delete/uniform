...
| `SH-12` | Implement MIR construction and verification in Gravity | `SH-11` | partial | Gravity builds verified C11 MIR for the whole bootstrap subset, including blocks, values, calls, branches, loops, errors, effects, ownership, safety facts, and source provenance; malformed MIR is rejected. The fixed Stage7 development lane now checks the frozen 253,588-byte C11 source and runs a six-selector C10-to-MIR adapter batch (helper first, authenticated boundary last); its 5/182 green receipt is non-authoritative and the C11 proof candidate remains manual and unattested. |
