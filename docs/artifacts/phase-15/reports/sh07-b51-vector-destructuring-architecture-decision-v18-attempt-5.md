# SH-07 B51 Let/Loop Vector Destructuring Architecture Decision V18, Attempt 5

Status: draft; not review-pending; implementation remains stopped

Date: 2026-08-29

## Purpose and disposition of attempts 1, 2, 3, and 4
...
integration, or SH-07 completion authority.

The attempt-4 report is retained at
`docs/artifacts/phase-15/reports/sh07-b51-vector-destructuring-architecture-decision-v18-attempt-4.md`.
Its exact candidate was commit
`c7fe6e0cbbba81df5a0cfdd5da286fc4047a99eb`, tree
`f0c6cf249e72b1f7050dbaa718a86a438d4d5384`, and report SHA-256
`8da37e14e0c9aeafd379b207113a31d2dc2cda902d72a216f7fda8eab3d7bdcb`.
Independent reviewer `b51_v18_attempt4_independent_review` rejected that exact
tuple, and governance commit
`4062d23eadf7d8d013e1163006423db416d88ee2` records its terminal rejection.
The reviewer recorded these exact three blockers verbatim:

```text
the pinned 849-edge predecessor carrier is described as a sorted vector but its 5f7a87 digest is the canonical hash of the sorted set; the catalog defines eight resource-bound checks with six predecessor-owned and two V18-owned but an acceptance clause says seven; the G and atomic-stack governance text names nonexistent dependency sh07-b51-v18-architecture-attempt-4 instead of the exact ledger id.
```

1. "the pinned 849-edge predecessor carrier is described as a sorted vector
   but its 5f7a87 digest is the canonical hash of the sorted set";
2. "the catalog defines eight resource-bound checks with six
   predecessor-owned and two V18-owned but an acceptance clause says seven";
   and
3. "the G and atomic-stack governance text names nonexistent dependency
   sh07-b51-v18-architecture-attempt-4 instead of the exact ledger id."

Attempt 4 is immutable terminal evidence and grants no implementation,
integration, or SH-07 completion authority. Attempt 5 is this draft
report-only successor. Its governance-history parent is exact commit
`4062d23eadf7d8d013e1163006423db416d88ee2`; it has not been frozen or
...
Attempt 5 retains those decisions and makes the closure preimage, parse-module
...
start until a future exact attempt-5 candidate is frozen, independently
...
parent is `4062d23eadf7d8d013e1163006423db416d88ee2`.
...
{:domain :gravity/sh07-b51-predecessor-executable-closure-v18
 ...
 :edge-count 849
 :edge-carrier predecessor-executable-edge-carrier
 :edge-carrier-kind :canonical-sorted-set-of-pair-vectors
 :edge-hash
...
caller/callee pairs. The unique predecessor executable edge carrier is
normatively the following value:

```text
predecessor-executable-edge-carrier :=
(sorted-set [caller-symbol callee-symbol] ... 849 distinct pair vectors ...)
```

It is a canonical sorted set, not a vector, sequence, call-site list, or
insertion-ordered collection. Each member is exactly a two-symbol vector; set
membership deduplicates the 1,290 encountered call sites before canonical
sorting. SHA-256 over the C11 canonical `pr-str` of this exact sorted-set value
is the preserved measured hash
`sha256:5f7a87d2a3fc9b43469c5a46109f7b1d8edd1860631d20508d6222a2c2f465ef`.
The carrier and hash remain summary evidence excluded from the already pinned
f473 descriptor preimage, so this correction changes no dependent pin.
Substituting a vector, retaining a duplicate call site, changing pair order or
member type, omitting a pair, or adding a pair is `C6-VERIFY`. The separately
named v18 `:runtime-call-edges` field and authenticated B47 call/call-edge
product vectors are ordered runtime or product records; neither is this
predecessor executable edge set and neither may be hashed as 5f7a87.
...
{:domain :gravity/sh07-b51-v18-executable-closure-carrier-v18
 ...
 :governance-parent-commit "4062d23eadf7d8d013e1163006423db416d88ee2"
 :physical-census
 {:source-byte-count nonnegative-integer
  :source-content-hash digest-id
...
The limits and their diagnostic ownership are closed by this table. It has
exactly eight checks/families: six `:predecessor-b47` rows and two `:v18`
rows. No row is an alias for another family and no ninth implicit check is
permitted. A
...
Attempt 5 does not expand source admission or lower a mandated B51 limit.
...
fail closed and no host exception may be passing evidence. Closure evidence
must also rebuild the exact 849-member predecessor sorted set and reject a
vector carrier, a duplicated call-site sequence, a non-pair member, a missing
or extra pair, or any resulting mismatch with preserved hash 5f7a87.
...
`sh07-b51-vector-destructuring-architecture-v18-attempt-5`, invariant family
...
base_commit      G
dependencies     [sh07-b51-vector-destructuring-architecture-v18-attempt-5]
candidate        H only
```

That dependency value must be exactly
`sh07-b51-vector-destructuring-architecture-v18-attempt-5`, byte-for-byte equal
to the integrated architecture ledger row's `:id`. A shortened alias, an
attempt-4 id, a title, a report path, or any other identifier is invalid. The
dedicated atomic-stack validator and H's independent reviewer compare the two
literal identifiers and include mutations for shortening, substituting the
attempt-4 id, changing one character, and naming a nonexistent row; every such
mutation must fail before implementation admission.
...
record whose `id` is exactly
`sh07-b51-vector-destructuring-architecture-v18-attempt-5` and whose
`base_commit` is the terminal attempt-4 governance parent
`4062d23eadf7d8d013e1163006423db416d88ee2` and whose candidate/tree are that
...
candidate descended from governance parent `4062d23...`. An independent
...
1. The v17 contradiction and exact attempt-1 rejection tuple are preserved;
   ...
   `5ec786d...`, with all three blocker strings retained verbatim; and attempt
   4's exact `c7fe6e0...` / `f0c6cf2...` / report SHA `8da37e...` tuple is
   terminally rejected by the independent review recorded at governance parent
   `4062d23...`, with all three blocker strings retained verbatim.
...
4. The measured predecessor closure has exact roots, 298 reachable members,
   ...
   hashes; its 849 unique caller/callee pairs are the canonical sorted set of
   two-symbol pair vectors whose preserved hash is exact 5f7a87, never an
   ordered edge or call-site vector. Closure drift or carrier-kind substitution
   fails before invocation. The combined C whole-file
...
10. Slot/extraction products retain empty vectors, all ordinals, origins,
    ...
    atomic transfer.
11. All eight resource-bound families have literal limits, saturating sources,
...
12. The architecture retains invariant family
    ...
    Its future ledger id and the atomic implementation dependency are both
    exactly `sh07-b51-vector-destructuring-architecture-v18-attempt-5`; alias,
    attempt-number, nonexistent-row, and one-character-drift mutations reject.
