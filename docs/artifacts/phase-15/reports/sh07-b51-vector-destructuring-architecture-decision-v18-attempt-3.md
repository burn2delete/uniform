# SH-07 B51 Let/Loop Vector Destructuring Architecture Decision V18, Attempt 3

Status: draft; not review-pending; implementation remains stopped

Date: 2026-08-29

## Purpose and disposition of attempts 1 and 2
...
input whose values may be silently changed. Attempt 3 is a draft report-only
successor whose governance-history parent is ce2a9b8; it has not been frozen
or submitted for review. Its semantic implementation base remains M.
...
Attempt 3 must close those seven blockers plus the exact-schema defects found
during its internal draft audit before it can be frozen and submitted to a
...
Attempt 3 retains those decisions and makes the closure preimage, parse-module
...
start until a future exact attempt-3 candidate is frozen, independently
...
parent is `ce2a9b8d5ee8e6a703894e5120dada36dd8ea182`.
...
{:domain :gravity/sh07-b51-v18-executable-closure-carrier-v18
 ...
 :governance-parent-commit "ce2a9b8d5ee8e6a703894e5120dada36dd8ea182"
 :physical-census
 {:source-byte-count nonnegative-integer
  :source-content-hash digest-id
...
3. sh07-b51-resolve-digest-preimage(purpose, preimage, resolved-prefix)
...
[{root sh07-b51-build-template
 ...
 {root sh07-b51-resolve-digest-preimage
  args [purpose preimage resolved-prefix]
...
Entrypoint 3 has this exact ABI. Its third argument is literally `[]` for the
...
Gravity validates purpose, domain, tier/cardinality, and prefix and returns
the accepted hash input; the host performs only the C11 hash and returns the
...
subtier/branch/cardinality catalog. The resolver checks all five columns
rather than looking up a merely
...
{:sh07-b51-implementation-semantic-id
 {:preimage-domain :gravity/sh07-b51-implementation-semantic-id-preimage-v18
  :tier 0 :subtier :0a :branch :both :cardinality :one-per-C}
 :sh07-b51-contract-semantic-id
 {:preimage-domain :gravity/sh07-b51-contract-semantic-id-preimage-v18
  :tier 0 :subtier :0a :branch :both :cardinality :one}
 ...
 {:preimage-domain :gravity/sh07-b51-independent-verifier-binding-preimage-v18
  :tier 5 :subtier nil :branch :both :cardinality :one}
 :sh07-b51-final-artifact-binding-id
 {:preimage-domain :gravity/sh07-b51-final-artifact-binding-preimage-v18
  :tier 6 :subtier nil :branch :both :cardinality :one}}
```

Tier 0 requests use their declared 0a/0b/0c/0d subtier; all later ordered
batches use `:subtier nil` and their dense global ordinal. A request whose
purpose, preimage domain, tier, subtier, branch, or cardinality does not match
this literal catalog is `C6-VERIFY`.
...
at global/subtier ordinal zero; a second closure, missing request, or one
...
The 0a subordinal is also literal and branch-sensitive:
...
exactly three; no other request may claim `:0a`. The 0b outcome, 0c authority, and 0d
product requests then follow their declared global ordinal and purpose map.

Requests are dense in `:ordinal`, dense within each non-nil subtier, and
topologically ordered. A request's `:tier`/`:subtier` must equal the declared
domain of its preimage; no request can mention its own or a descendant id. The
...
[[:destructuring-product-nodes :each :node-id]
 ...
 [:destructuring-product-nodes :each :recur-coordinate :recur-core-node-id :optional]
...
{:legacy-v16-accepted
 {:concrete (concat inherited-product-reference-paths
                    [legacy-inherited-artifact-reference-path])
  :generated (concat b51-product-reference-paths
                     legacy-generated-reference-paths)}
...
{:sh07-b51-implementation-semantic-id []
 ...
 :sh07-b51-core-node-id
 [[:predecessor-authority-id] [:children :each]
  [:evaluated-children :each]]
 :sh07-b51-product-node-id [[:predecessor-authority-id]]
 :sh07-b51-binding-slot-id
 [[:predecessor-authority-id] [:owner-core-node-id]
  [:initializer-core-node-id]]
 :sh07-b51-binding-extraction-id
 ...
 [[:predecessor-authority-id] [:slot-id :optional] [:slot-ids :each]
  ...
  [:recur-coordinate :recur-core-node-id :optional]]
 :sh07-b51-recur-slot-mapping-id
 [[:predecessor-authority-id] [:loop-core-node-id] [:recur-core-node-id]
  [:target-loop-core-node-id] [:commit-publication-id] [:slot-ids :each]
  [:argument-node-ids :each] [:extraction-id-vectors :each :each]
...
ancestor positions in `preimage-reference-registry`; therefore the old claim
...
v16 digests. Rejected template roots and generated product paths contain exact
internal references. Resolved cores and outputs require concrete digest ids
...
`:output-def-syntax-id` equals `R.syntax-id`. The general-root branch requires
`Tmatches` to be empty and requires that `R`'s own `:generated-origin` vector
contain no `:introduced-def` entry; its defn-input, def, and fn coordinate
fields are all nil. If `Tmatches` has exactly one entry `T`, the expanded-defn
subcase requires `R` to be an authenticated `def` with exactly three children,
`child[2]` to be an `fn` with
...
separate or inferred origin table. A missing/ambiguous trace or origin, or a
tagged expanded root with the wrong def/fn shape, fails `C6-ORIGIN` or
`C6-CORE-SHAPE` respectively; more than one matching trace also fails closed.
...
For the expanded-defn subcase, `root-form-id = def-form-id` and
`root-syntax-id = def-syntax-id`; for a source-defined or other top-level root,
all five defn/def/fn fields are nil. Coordinates are joined to the exact
...
coordinate, or a non-nil field outside the expanded-defn subcase, fails closed.
...
The following seven id-seed/preimage maps are closed. They are emitted in
ordered batches with `:subtier nil` (the only subtiers are 0a/0b/0c/0d):
...
recur, or descendant ids. Tier 1 emits all slot seeds before extraction
...
extraction path. `:recur-form-id` and `:recur-syntax-id` are both required or
...
`resolved-prefix []` by passing the request's purpose and preimage as the first
two arguments. Gravity returns `digest-preimage-resolution`, the host computes
...
`sh07-b51-v18-architecture-attempt-3`, invariant family
...
base_commit      G
dependencies     [sh07-b51-v18-architecture-attempt-3]
candidate        H only
```
...
record whose `base_commit` is the terminal attempt-2 governance parent
`ce2a9b8d5ee8e6a703894e5120dada36dd8ea182` and whose candidate/tree are that
...
candidate descended from governance parent `ce2a9b8...`. An independent
...
1. The v17 contradiction and exact attempt-1 rejection tuple are preserved;
   ...
   by the independent review recorded at governance parent `ce2a9b8...`, with
   all seven blockers retained without relabeling.
