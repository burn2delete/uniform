...
`gravity.workflow` defines durable workflows, steps, recorded events, activities, retries, timers, compensation, child workflows, approvals, replay, and workflow schemas.
...
Time, random values, network calls, database calls, model calls, file IO, approvals, and external signals are recorded, isolated, or rejected.
...
- Timers, signals, approvals, external callbacks, and child workflows MUST become typed events.
...
- AI model calls and tool calls inside workflows MUST preserve Phase 11 policy, `:ai/human-approval`, budget, memory, and eval records.
...
- Approvals: `approval-step`, `approval-policy`, and `approval-event`.
...
- `A1`, `A5`, `A6`, `A9`, and `A10` for AI model, agent, workflow, eval, and approval semantics.
...
- `:ai` receives workflow APIs when agent execution requires durable state, tools, memory, `:ai/human-approval`, or eval records.
...
- AI policy, `:ai/human-approval`, budget, tool, memory, and eval records when workflows call AI APIs.
...
- `STD12005` when a timer, signal, approval, or callback is not represented as a typed event.
- `STD12006` when workflow version changes cannot migrate in-flight state.
- `STD12007` when AI calls bypass Phase 11 policy, `:ai/human-approval`, or eval records.
...
- AI workflow fixtures preserve model, tool, memory, policy, `:ai/human-approval`, budget, and eval evidence.
