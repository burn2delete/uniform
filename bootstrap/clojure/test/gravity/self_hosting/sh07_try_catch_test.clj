...
(def ^:private rejected-oracles
  {"missing-catch"
   ...
   {:rule "C6-LOWERING-GAP"
    ...
    :remediation
    "Wrap protected sequencing in an explicit do form; SH-07-B10 accepts one protected expression."}
   "handler-sequencing-deferred"
   {:rule "C6-LOWERING-GAP"
    :reason :catch-handler-sequencing-deferred
    :remediation
    "Wrap handler sequencing in an explicit do form; SH-07-B10 accepts one handler expression per catch clause."}
   "finally-remains-deferred"
   {:rule "C6-LOWERING-GAP"
    :reason :finally-clause-deferred
    :remediation
    "Omit finally in SH-07-B10; defer cleanup lowering to its owning slice."}})
