(ns gravity.self-hosting.sh07-c4-compat-source-coverage-test
...
(defn- repository-root
  ...
  (let [resource
        (io/resource
         "gravity/self_hosting/sh07_c4_compat_source_coverage_test.clj")]
    (when-not resource
      (throw
       (ex-info
        "SH-07 C4 compatibility source test is not on the classpath"
        {:id "SH07-C4-COVERAGE-TEST-SOURCE"})))
      ...
      (cond
        ...
        (throw
         (ex-info
          ...
          {:id "SH07-C4-COVERAGE-REPOSITORY-ROOT"}))
...
(def ^:private expected-source-byte-count 4053)
...
(def ^:private expected-definition-names
  '#{c4-macro-expansion-compatibility-contract
     c4-exact-keys?
...
(defn- required-var
  ...
  (or (ns-resolve 'gravity.bootstrap symbol)
      (throw
       (ex-info
        "Required SH-07-B20 coordinator adapter is absent"
        {:id "SH07-C4-COVERAGE-ADAPTER-ABSENT"
         :symbol symbol}))))
...
(defn- exactly-once-index
  ...
  (let [index (into {} (map (juxt key-name identity)) records)]
    (when-not (= (count records) (count index))
      (throw
       (ex-info
        "SH-07-B20 records are not uniquely identifiable"
        {:id "SH07-C4-COVERAGE-AMBIGUOUS-INDEX"
         :key key-name
...
(def ^:private c4-artifact
  (delay
    ((required-var 'sh07-core-file-artifact)
     (path c4-relative-path))))

(def ^:private c4-upstream-verification
  (delay
    ((required-var 'sh06-resolution-artifact-verification)
     (:sh06-resolution-artifact @c4-artifact))))

(def ^:private parity-artifacts
  (delay
    (let [temp-root
          (java.nio.file.Files/createTempDirectory
           "gravity-sh07-c4-coverage-"
           (make-array java.nio.file.attribute.FileAttribute 0))
          right-path (.resolve temp-root "right/c4_macro_expansion_engine.qst")
          left-path (path c4-relative-path)
...
(deftest sh07-b20-proof-contract-registers-c4-source-exactly
  (let [contract
        (edn/read-string
         (slurp (path proof-contract-relative-path)))
        documents (set (:governing-documents contract))]
    (is (= "SH-07-B20" (:coverage-milestone contract)))
    (is (= c4-relative-path
           (get-in contract [:authoritative-modules :c4-compat])))
    (is (= {:keyword-lookups 0}
           (get-in contract
                   [:required-core-product-counts :c4-compat])))
    (doseq [document
    ...
    (is (= expected-source-byte-count
           (alength (source-bytes (path c4-relative-path)))))
    (is (= expected-source-revision-id
           (sha256-id (source-bytes (path c4-relative-path)))))
    (is (contains? (set (:nonclaims contract))
                   :c4-adapter-retirement))
    (is (contains? (set (:nonclaims contract)) :sh07-complete))))

(deftest sh07-b20-c4-source-has-exact-authentic-coverage
  (let [artifact @c4-artifact
    ...
    (is (= 'gravity.compiler.c4-macro-expansion-engine
...
(deftest sh07-b20-c4-definitions-calls-and-lookup-boundary-are-exact
  (let [core-artifact (core @c4-artifact)
        nodes (:nodes core-artifact)
    ...
    (is (= 3
           (count (filter #(= :fn (:core-form %)) nodes))))
    (is (= 13
           (count (filter #(= :if (:core-form %)) nodes))))
    ...
    (doseq [call calls]
      (let [call-node (get node-by-id (:core-node-id call))
        ...
        (is (map? operator))
        (is (= :operator (:position operator)))
        (is (= :call (:core-form call-node)))
        (is (= (:operator-node-id call)
               (first (:children call-node))))
        (is (= :operator-then-arguments
               (:evaluation-order call)
               (get-in call-node [:evaluation :kind])))))))

(deftest sh07-b20-c4-is-deterministic-path-neutral-and-provenanced
...
(deftest sh07-b20-c4-replay-and-alteration-containment-pass
  (let [artifact @c4-artifact
        report
            ...
            [["call binding"
             ...
             ["actual path provenance"
              (assoc-in
               ...
               "/altered/root/c4_macro_expansion_engine.gravity")
              :provenance-retained?]
             ["stored proof"
      ...
      (testing label
        (let [checks
              ((required-var 'sh07-core-verification-checks)
               altered artifact @c4-upstream-verification)
              failed
              (set
...
(deftest sh07-b20-existing-rejected-families-remain-paired-and-structured
