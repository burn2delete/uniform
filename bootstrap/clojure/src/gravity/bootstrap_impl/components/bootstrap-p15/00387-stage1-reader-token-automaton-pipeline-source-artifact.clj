...
(defn stage1-reader-token-automaton-pipeline-source-artifact
  ...
  (let [stage1-bootstrap-artifact
        ...
        (binding [*stage1-reader-pipeline-trace* trace]
          (stage1-reader-execute-gravity-token-automaton-pipeline
        ...
        artifact-base
        {:kind :gravity/stage1-reader-token-automaton-pipeline-artifact
         :phase "15"
         :task "P15-S8"
         ...
         :stage1-reader-token-automaton-pipeline-trace
         ...
         :accepted-stage1-reader-token-automaton-pipeline-fixtures
         ...
         :rejected-stage1-reader-token-automaton-pipeline-fixtures
         stage1-reader-token-automaton-pipeline-rejected-fixture-records
         :stage1-reader-token-automaton-pipeline-diagnostic-stream
         (stage1-reader-token-automaton-pipeline-diagnostic-stream
          source-path token-automaton-pipeline-id)
         :stage1-reader-token-automaton-pipeline-results
         {:accepted-fixtures 1
          :rejected-fixtures
          (count stage1-reader-token-automaton-pipeline-rejected-fixture-records)
          :diagnostic-count
          (+ (count stage1-reader-token-automaton-pipeline-diagnostic-ids)
...
(defn stage1-reader-token-automaton-pipeline-file-artifact
  [path]
  (stage1-reader-token-automaton-pipeline-source-artifact path (slurp path)))

(defn stage1-reader-execute-gravity-form-builder-pipeline
  ...
  (let [forms (stage1-reader-source-forms reader-source-path)
    ...
    (when-not (= :defn (:kind (get definitions
      ...
      (stage1-reader-form-builder-pipeline-fail!
       "STAGE1FORM001" reader-source-path
       stage1-reader-form-builder-pipeline-entrypoint
       {:missing-fields [stage1-reader-form-builder-pipeline-entrypoint]}))
    (stage1-reader-execute-gravity-function
     reader-source-path definitions
     stage1-reader-form-builder-pipeline-entrypoint
     [source-path source-text])))

(defn stage1-reader-form-builder-pipeline-diagnostic-stream
  [source-path form-builder-pipeline-id]
  {:artifact :gravity/stage1-reader-form-builder-pipeline-diagnostic-stream
   ...
   (mapv (fn [id]
           {:diagnostic-id (str "diag-" (str/lower-case id))
            ...
            :message
            (or (stage1-reader-form-builder-pipeline-diagnostic-messages id)
                (stage1-reader-execution-diagnostic-messages id))})
         (concat stage1-reader-form-builder-pipeline-diagnostic-ids
                 (butlast stage1-reader-execution-diagnostic-ids)))
   :status :complete})

(defn stage1-reader-form-builder-pipeline-proof
  [artifact]
  (let [diagnostics (set (map :diagnostic
    ...
    {:gravity-reader-form-builder-pipeline-entrypoint-executed?
     (= stage1-reader-form-builder-pipeline-entrypoint
     ...
     :form-builder-covered?
     (and (= :gravity-reader-form-builder-v1 (:engine form-builder))
          (= :stage1-reader-token-automaton (:derived-from form-builder))
          (= (:engine token-automaton)
             (:token-automaton-engine form-builder))
          (= :gravity-source (get-in form-builder [:provenance :owner]))
          (contains? (:diagnostics form-builder) :invalid-form-builder)
          (set/subset? #{:list :vector :map :set}
                       (set (:collection-kinds form-builder)))
          (set/subset? #{:build-list-form :build-vector-form
     ...
     :diagnostics-covered?
     (set/subset? (set (concat stage1-reader-form-builder-pipeline-diagnostic-ids
                                (butlast stage1-reader-execution-diagnostic-ids)))
                  diagnostics)
     :limitations
     {:clojure-seed-evaluator? true
      ...
      :host-form-builder-executor? true
      :clojure-seed-retired? false
      :next-required-capability
      :replace-token-automaton-and-form-builder-executors-with-gravity-code}
     :status :complete}))