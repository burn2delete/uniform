...
(defn stage1-reader-token-classifier-pipeline-source-artifact
  ...
  (let [stage1-bootstrap-artifact
        ...
        (binding [*stage1-reader-pipeline-trace* trace]
          (stage1-reader-execute-gravity-token-classifier-pipeline
        ...
        artifact-base
        {:kind :gravity/stage1-reader-token-classifier-pipeline-artifact
         :phase "15"
         :task "P15-S6"
         ...
         :stage1-reader-token-classifier-pipeline-trace
         (dissoc trace-value :character-stream :token-stream
                 :token-classifier)
         :stage0-comparison comparison
         :accepted-stage1-reader-token-classifier-pipeline-fixtures
         ...
         :rejected-stage1-reader-token-classifier-pipeline-fixtures
         stage1-reader-token-classifier-pipeline-rejected-fixture-records
         :stage1-reader-token-classifier-pipeline-diagnostic-stream
         (stage1-reader-token-classifier-pipeline-diagnostic-stream
          source-path token-classifier-pipeline-id)
         :stage1-reader-token-classifier-pipeline-results
         {:accepted-fixtures 1
          :rejected-fixtures
          (count stage1-reader-token-classifier-pipeline-rejected-fixture-records)
          :diagnostic-count
          (+ (count stage1-reader-token-classifier-pipeline-diagnostic-ids)
...
(defn stage1-reader-token-classifier-pipeline-file-artifact
  [path]
  (stage1-reader-token-classifier-pipeline-source-artifact path (slurp path)))

(defn stage1-reader-execute-gravity-token-realizer-pipeline
  ...
  (let [forms (stage1-reader-source-forms reader-source-path)
    ...
    (when-not (= :defn (:kind (get definitions
      ...
      (stage1-reader-token-realizer-pipeline-fail!
       "STAGE1REAL001" reader-source-path
       stage1-reader-token-realizer-pipeline-entrypoint
       {:missing-fields [stage1-reader-token-realizer-pipeline-entrypoint]}))
    (stage1-reader-execute-gravity-function
     reader-source-path definitions
     stage1-reader-token-realizer-pipeline-entrypoint
     [source-path source-text])))

(defn stage1-reader-token-realizer-pipeline-diagnostic-stream
  [source-path token-realizer-pipeline-id]
  {:artifact :gravity/stage1-reader-token-realizer-pipeline-diagnostic-stream
   ...
   (mapv (fn [id]
           {:diagnostic-id (str "diag-" (str/lower-case id))
            ...
            :message
            (or (stage1-reader-token-realizer-pipeline-diagnostic-messages id)
                (stage1-reader-execution-diagnostic-messages id))})
         (concat stage1-reader-token-realizer-pipeline-diagnostic-ids
                 (butlast stage1-reader-execution-diagnostic-ids)))
   :status :complete})

(defn stage1-reader-token-realizer-pipeline-proof
  [artifact]
  (let [diagnostics (set (map :diagnostic
    ...
    {:gravity-reader-token-realizer-pipeline-entrypoint-executed?
     (= stage1-reader-token-realizer-pipeline-entrypoint
     ...
     :diagnostics-covered?
     (set/subset? (set (concat stage1-reader-token-realizer-pipeline-diagnostic-ids
                                (butlast stage1-reader-execution-diagnostic-ids)))
                  diagnostics)
     :limitations
     {:clojure-seed-evaluator? true
      :host-character-stream? true
      :host-token-realizer-executor? true
      :host-form-builder? true
      :clojure-seed-retired? false
      :next-required-capability
      :replace-token-realizer-executor-and-form-builder-host-primitives-with-gravity-code}
     :status :complete}))