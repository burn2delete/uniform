...
(clojure.core/defn
 ...
 (clojure.core/let
  [{:keys
   ...
   (cond->
    {:source-map-hash source-map-hash,
     ...
     :kind :gravity/c-backend-provenance,
     :compiler {:owner :clojure-bootstrap, :stage :stage0, :plan-hash plan-hash},
     :final-release? false,
     :target {:backend :c, :dialect dialect, :target target},
     :stage2-runtime-execution-record
     (when
      runtime-derived?
  ...
  (clojure.core/assoc state :provenance provenance :provenance-hash provenance-hash)))
