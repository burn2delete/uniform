(ns gravity.c15-diagnostics.operations)
...
(defn invoke [key fallback & args]
  (if-let [operation (when-not (contains? *active-operation-keys* key) (get *operations* key))]
    (binding [*active-operation-keys* (conj *active-operation-keys* key)] (apply operation args))
    (apply fallback args)))
